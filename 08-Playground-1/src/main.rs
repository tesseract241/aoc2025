...
fn main() {
    ...
    for couple in couples.iter().take(total_connections) {
    ...
    let mut circuit_sizes = circuits.iter().map(|x| x.len()).collect::<Vec::<usize>>();
    circuit_sizes.sort_unstable_by_key(|x| -(*x as i64));
    output = circuit_sizes[..largest_circuits].iter().product();
    println!("{output}");
